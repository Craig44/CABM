...
\paragraph{Baranov}\label{para:baranov}\index{Processes!Mortality!Baranov}
...
\end{verbatim}}}
\paragraph{Tag Recaptures}\label{subsubsec:tag_recaptures}\index{Processes!Mortality!Tag Recaptures}
Because the underlying model entity is an agent than often represents many individuals and a tagged entity is an individual careful consideration is needed not to introduce bias quantities. The first thing to note that users must specify a tagging event Section~\ref{subsub:tag} so that tagged fish exist in the partition. Otherwise this will be a computational burden. The algorithm for observing tag recaptures during a removal process follows;

\begin{enumerate} 
	\item Calculate proportion of tagged \textbf{individuals} from the vulnerable population, denoted \(P_t\). Note vulnerable is a stochastic evaluation see the above mortality processes e.g. \(U() \leq P_a\)\\
	\item When in a removal process and we are scanning catch for tags, randomly draw from a Bernoulli distribution.
	\[\text{select tagged fish }= Binomial(1,P_a) \]
	\item In most mortality processes users must specify the proportion of the catch scanned for tags (\(P_{scan}\)). So if a tagged fish is selected. \IBM\ then does a check to see if we observed that recapture i.e. \(U() \leq P_{scan}\).
\end{enumerate}

There is two ways to get \IBM\ to report information on tag-recaptures. You can either generate a \subcommand{tag\_recapture\_by\_length} or \subcommand{tag\_recapture\_by\_age} observation class (See sections~\ref{subsubsec:tag_recap_by_age} \& \ref{subsubsec:tag_recap_by_length}), or generate a report class from the mortality process that the recaptures occurred in.
