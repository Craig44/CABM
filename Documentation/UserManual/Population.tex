...
Currently users can only initialise the partition an iterative process. \IBM\ does a few tricks to help speed up the initialisation. The first thing \IBM\ does is gets the parameter \subcommand{number\_of\_agents} and spits that number of agents uniformly, over the spatial domain. It then

The other way is, if users play around with a model and have an idea of what the spatial distribution of the initial state is, they could specify a layer that would, not split them uniformly to speed up this set up the initial phase.
