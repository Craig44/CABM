...
if (grepl("unix", .Platform$OS.type)) {
  ...
  VERSION<-system("./cabm -v",intern=TRUE)
  setwd(this_dir)
} else {
  VERSION<-system("cabm.exe -v",intern=TRUE)
...
cat("Author: C. Marsh\n",file=filename,append=TRUE)
cat("Description: A set of R functions for extracting and plotting from cabm output files.\n",file=filename,append=TRUE)
cat("Maintainer: Craig Marsh <craig.marsh10@gmail.com>\n",file=filename,append=TRUE)
...
filename<-"ibm/R/CABM.binary.version.R"
cat("\"CABM.binary.version\"<-\n",file=filename)
