...
#' @title get_numeric_layer
#'
#' @description
#' return a long data frame of numeric layers
#'
#' @author Craig Marsh
#' @param model <ibm_output> object that are generated from one of the extract() functions.
#' @param report_label <string>
#' @param row_labs vector of strings first
#' @param col_labs vector of strings first
#' @return a data frame
#' @export 
...
get_numeric_layer = function(model, report_label, file_name = "", breaks = NULL, col_labs = NULL, row_labs = NULL) {
  ...
  for (i in 1:length(report)) {
    if( names(report)[i] != "type") {
      this_data = reshape2::melt(report[[i]]$values)
      if(!is.null(row_labs)) {
        this_data$Var1 = row_labs[this_data$Var1]
        this_data$Var1 = factor(this_data$Var1, levels = rev(row_labs), ordered = T)
      } else {
        change_y_for_plotting = rev(sort(unique(this_data$Var1)))
        this_data$Var1 = change_y_for_plotting[this_data$Var1]
      }
      if(!is.null(col_labs)) {
        this_data$Var2 = col_labs[this_data$Var2]
        this_data$Var2 = factor(this_data$Var2, levels = col_labs, ordered = T)
      }
      brks <- cut(this_data$value,breaks)
      brks = factor(brks, levels=levels(brks))
      this_data$brks <- brks
      full_df = rbind(full_df, this_data)
    }
  }
  return(full_df)
}
...
plot_numeric_layer = function(model, report_label, directory = "", file_name = "", Title = "", breaks = NULL, width = NA, height = NA, col_labs = NULL, row_labs = NULL) {
  ## check report label exists
  if (!report_label %in% names(model))
    stop(Paste("In model the report label '", report_label, "' could not be found. The report labels available are ", paste(names(model),collapse = ", ")))
  ## get the report out
  ...
  for (i in 1:length(report)) {
    if( names(report)[i] != "type") {
      ...
      brks = factor(brks, levels=levels(brks))
      this_data$brks <- brks
      P = ggplot(this_data, aes(y = Var1, x = Var2,fill=brks))  + geom_tile() +
