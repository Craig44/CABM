#' @title plot_spatial_fishery_attributes
#'
#' @description
#' A plotting function to generate a snapshot of heatmaps that can be used in an animation, to show how the 
#' where the vulnerable biomass and where the catches were, from a mortality_effort_based and process
#'
#' @author Craig Marsh
#' @param model <ibm_output> object that are generated from one of the extract() functions.
#' @param report_label <string>
#' @param pattern a string that the function will do grepl call on to get the matrix to generate a plot
...
#' @export 
plot_spatial_fishery_attributes = function(model, report_label, directory = "", pattern = "removals_", file_name = "", Title = "", breaks = NULL, width = NA, height = NA, plot_catch = T) {
