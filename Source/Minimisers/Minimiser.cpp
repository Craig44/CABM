...
void Minimiser::PassBaranovProcess(string baranov_process_label) {
  mortality_process_ = model_->managers().process()->GetMortalityProcess(baranov_process_label);
  if(!mortality_process_) {
    LOG_FATAL() << "could not build baranov pointer";
  }
}
