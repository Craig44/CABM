...
void Engine::buildScaledValues() {
  ...
  for (int i = 0; i < (int)vStartValues.size(); ++i) {
    LOG_FINE() << "looking at parameter = " << i + 1 << " " << vStartValues[i] << " lower bound = " << vLowerBounds[i] << " Upper bound = " <<  vUpperBounds[i];
    // Check
...
//**********************************************************************
// Overload the above function for Baranov
//**********************************************************************
double Engine::optimise_finite_differences(gammadiff::CallbackBaranov& objective, vector<double>& StartValues, vector<double>& LowerBounds,
    vector<double>& UpperBounds, int& convergence, int& iMaxIter, int& iMaxFunc, double dGradTol,
    double **pOptimiseHessian, int untransformedHessians, double dStepSize) {
  LOG_FINE() << "entering optimiser";
  // Variables
