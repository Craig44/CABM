  ...
  def start(self):    
    ...
    if not os.path.exists(boostFileName):       
        ...
        if os.path.exists(boostFileName + '_windows.zip'):
            os.system('unzip ' + boostFileName + '_windows.zip 1> cabm_unzip.log 2>&1')      
    
    # Build Boost
    print '-- Building Boost - check cabm_build.log for progress - estimated time 30 minutes'
    os.chdir(boostFileName)
    os.system('bootstrap.bat gcc 1> cabm_bootstrap.log 2>&1')
    os.system('b2.exe --toolset=gcc runtime-link=static threading=multi 1> cabm_build.log 2>&1')
