...
Look natural morality values assigned to each agent, which is generated from a distribution in this case lognormal with expectation 0.13, and cv 0.15 (see line 102 in \texttt{Population.ibm}). When each agent is created it is assigned a random \(M\) value that is used in the natural mortality process. This is the same as how \(L_{\infty}\) and \(k\) are assigned in the growth function. 
```{r Attributes}
...
Look at Tag release distirbutions
```{r Tag release}
...
The cluster method doesn't seem to be describing the plus group well, best to look at many simualtions and take an average. This can be done using the \texttt{ibm -s} command in the ABM. See the file \texttt{simulated\_obs.log} to see the command call.
...
```
```{r read in simulated obs, include = FALSE}
